...
-- Storage policies for submitted photos and videos
...
DROP POLICY IF EXISTS "Guest upload media with token" ON storage.objects;
...
CREATE POLICY "Guest upload media with token"
  ...
  WITH CHECK (
    bucket_id = 'media'
    AND COALESCE(current_setting('request.headers', true)::json ->> 'x-guest-token', '') = public.get_guest_password()
  );
