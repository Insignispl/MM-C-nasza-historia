...
export default function EventManager({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  async function save() {
    if (!event) return;
    setSaving(true);
    const { error } = await supabase.from("events").update({ status: event.status, allow_uploads: event.allow_uploads, require_moderation: event.require_moderation, kiosk_enabled: event.kiosk_enabled, live_wall_enabled: event.live_wall_enabled, guest_reactions_enabled: event.guest_reactions_enabled, kiosk_countdown_seconds: event.kiosk_countdown_seconds, kiosk_burst_count: event.kiosk_burst_count, kiosk_frame_style: event.kiosk_frame_style }).eq("id", event.id);
