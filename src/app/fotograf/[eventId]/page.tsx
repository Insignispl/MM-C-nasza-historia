...
export default function EventManager({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  return <main className="min-h-screen px-4 pb-20 pt-28"><div className="mx-auto max-w-5xl">
    ...
    <aside className="space-y-5"><div className="rounded-3xl border border-primary/15 bg-primary/[0.06] p-6"><span className="inline-flex h-11 w-11 items-center justify-center rounded-2xl bg-white text-primary"><QrCode className="h-5 w-5" /></span><h2 className="mt-4 font-serif text-2xl">Link dla gości</h2><p className="mt-2 text-sm text-muted-foreground">Udostępnij go przez kod QR na stolikach.</p><div className="mt-4 break-all rounded-xl border border-primary/15 bg-white/80 p-3 text-xs">{guestUrl}</div><Button onClick={() => copy(guestUrl)} variant="outline" className="mt-3 w-full gap-2"><Copy className="h-4 w-4" /> Kopiuj link</Button></div><Link href={`/e/${event.slug}/live`}><Button variant="outline" className="w-full gap-2"><MonitorPlay className="h-4 w-4" /> Otwórz Live Wall</Button></Link><Link href={`/e/${event.slug}`}><Button variant="outline" className="w-full gap-2"><Sparkles className="h-4 w-4" /> Podgląd strony gościa</Button></Link></aside></div>
  </div></main>;
}
