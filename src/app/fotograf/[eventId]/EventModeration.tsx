...
export function EventModeration({ eventId }: { eventId: string }) {
  ...
  const fetchItems = useCallback(async () => {
    ...
    return [...(media || []).map((item) => ({ ...item, kind: "media" as const })), ...(entries || []).map((item) => ({ ...item, kind: "entry" as const }))].sort((a, b) => +new Date(b.created_at) - +new Date(a.created_at));
  }, [eventId, supabase]);

  useEffect(() => {
    let mounted = true;
    fetchItems().then((data) => { if (mounted) { setItems(data); setLoading(false); } });
