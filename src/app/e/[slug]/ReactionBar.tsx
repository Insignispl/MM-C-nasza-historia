...
export function ReactionBar({ eventId }: { eventId: string }) {
  ...
  return <section className="mx-auto max-w-3xl px-4 py-10 text-center"><div className="rounded-[2rem] border border-primary/10 bg-card/80 p-6 shadow-sm"><span className="inline-flex h-11 w-11 items-center justify-center rounded-2xl bg-primary/10 text-primary"><Heart className="h-5 w-5 fill-current" /></span><h2 className="mt-3 font-display text-3xl">Poczuj tę chwilę razem z Nimi</h2><p className="mt-2 text-sm text-muted-foreground">Zostaw reakcję dla Pary Młodej.</p><div className="mt-5 flex flex-wrap justify-center gap-2">{emojis.map((emoji) => <button key={emoji} onClick={() => react(emoji)} disabled={sending !== null} className="inline-flex min-w-16 items-center justify-center gap-2 rounded-2xl border border-border bg-card px-4 py-3 text-lg shadow-sm transition hover:-translate-y-1 hover:border-primary/30 hover:shadow-md disabled:opacity-60"><span>{emoji}</span><span className="text-xs font-semibold text-muted-foreground">{counts[emoji]}</span></button>)}</div></div></section>;
}
