...
export default function PhotoBoothPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  /**
   * Osiem sekund zyczen - rowniez w oprawie. Wczesniej nagrywalismy surowy obraz
   * z kamery, wiec kazde zdjecie z fotobudki mialo imiona i date, a film nie mial nic.
   * Teraz kazda klatka idzie przez te sama funkcje co zdjecie, wiec pamiatka jest jedna.
   */
  function recordVideo() {
    const video = videoRef.current;
    const canvas = canvasRef.current;
    const kamera = streamRef.current;
    if (!video || !canvas || !kamera || !video.videoWidth) return;
    chunksRef.current = [];

    let petla = 0;
    const rysujKlatke = () => {
      drawMountedPhoto(canvas, video, video.videoWidth, video.videoHeight, frameOpts);
      petla = window.requestAnimationFrame(rysujKlatke);
    };
    rysujKlatke();

    // captureStream z plotna oddaje TYLKO obraz. Bez dolozenia sciezki dzwieku
    // z mikrofonu film wychodzilby niemy, a to nagranie zyczen - dzwiek jest w nim
    // wazniejszy od obrazu.
    const zapis = canvas.captureStream(30);
    kamera.getAudioTracks().forEach((sciezka) => zapis.addTrack(sciezka));

    const recorder = new MediaRecorder(zapis, { mimeType: MediaRecorder.isTypeSupported("video/webm;codecs=vp9,opus") ? "video/webm;codecs=vp9,opus" : "video/webm" });
    recorderRef.current = recorder;
    recorder.ondataavailable = (event) => { if (event.data.size) chunksRef.current.push(event.data); };
    recorder.onstop = () => {
      window.cancelAnimationFrame(petla);
      preview(new Blob(chunksRef.current, { type: "video/webm" }), "video");
    };
    recorder.start();
    setStatus("recording");
