...
export default async function LiveWallPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const guestUrl = `${process.env.NEXT_PUBLIC_BASE_URL ?? "http://localhost:3000"}/e/${slug}/dodaj`;
  return <><ThemeController surface="live" /><LiveWall eventId={event.id} coupleName={event.couple_name} primaryColor={event.primary_color} initialMedia={media || []} guestUrl={guestUrl} /></>;
}
